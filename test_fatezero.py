...
from video_diffusion.models.estimator import MyUNetPseudo3DConditionModel
...
#     print("test videomodel0")
#     from IPython import embed; embed()
    estimator = MyUNetPseudo3DConditionModel.from_2d_model(
        os.path.join(pretrained_model_path, "unet"), model_config=model_config
    )
    ...
    pipeline.scheduler.set_timesteps(editing_config['num_inference_steps']) # DDIMScheduler
    ...
    estimator.to(accelerator.device, dtype=weight_dtype)
    ...
    if editing_config.get('use_invertion_latents', False):
        ...
        use_inversion_attention =  editing_config.get('use_inversion_attention', False) # True
        batch['latents_all_step'] = pipeline.prepare_latents_ddim_inverted(
            ...
            store_attention=use_inversion_attention, #True
            LOW_RESOURCE = True, # not classifier-free guidance
            save_path = logdir if verbose else None
            ) # 这一步获得inverted latents
        # torch.save(batch['latents_all_step'],"ddim_inversion_latent.pth")
        
        # from IPython import embed;embed()
        # batch['latents_all_step'] = torch.load("ddim_inversion_latent.pth")
    ...
    masks = rearrange(masks, "b c f h w -> (b f) c h w")
    ...
    if accelerator.is_main_process:
        ...
        if validation_sample_logger is not None:
            unet.eval()
            # video_diffusion.pipelines.p2p_validation_loop.P2pSampleLogger
            # 这一步很关键
            validation_sample_logger.log_sample_images( 
                image=images, # torch.Size([8, 3, 512, 512])
                mask=masks, # torch.Size([8, 1, 512, 512])
                pipeline=pipeline,
                estimator = estimator,
                device=accelerator.device,
                step=0,
                latents = batch['ddim_init_latents'], # torch.Size([1, 4, 8, 64, 64])
                ddim_latents = batch['latents_all_step'], # 51 [1, 4, 8, 64, 64]
