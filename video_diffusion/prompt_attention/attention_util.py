...
class AttentionControlEdit(AttentionStore, abc.ABC):
    ...
    def forward(self, attn, is_cross: bool, place_in_unet: str,judge=False):
        super(AttentionControlEdit, self).forward(attn, is_cross, place_in_unet)
        if attn.shape[-2] <= 32 ** 2:
            key = f"{place_in_unet}_{'cross' if is_cross else 'self'}"
            current_pos = self.attention_position_counter_dict[key]

            if self.use_inversion_attention: # 0 timestep时是True
...
def make_controller(tokenizer, prompts: List[str], is_replace_controller: bool,
    ...
    # from IPython import embed;embed()
    if (blend_words is None) or (blend_words == 'None'): # 不配置的情况下是对的
        latent_blend = None
    ...
    if equilizer_params is not None: #这步是True
