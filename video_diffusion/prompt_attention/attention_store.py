...
class AttentionStore(AttentionControl):
    """这个类 AttentionStore 继承了 AttentionControl，主要用于在模型的反向扩散过程中存储注意力图和潜在表示（latents）。该类实现了以下功能：

	1.	存储当前步骤的潜在表示（latent）：在每一步（step）完成后，存储生成的潜在表示。
	2.	存储和管理注意力图：存储每一步生成的注意力图，并在多个步骤之间进行累积和平均。
	3.	支持将注意力图存储在磁盘：可以选择将注意力图存储在磁盘中，以减少内存占用。
    """
    def step_callback(self, x_t):
